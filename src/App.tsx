...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"scan" | "diary" | "profile">(
    "scan",
  );
  ...
  const [diaryItems, setDiaryItems] = useState<FoodLogItem[]>([
    {
      ...
      brand: "Silk",
      loggedAt: new Date(Date.now() - 3600000 * 3).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      }),
      servingsCount: 1,
  ...
  const startCamera = async () => {
    try {
      ...
      console.warn(
        "Camera initialization failed, fallback to simulation:",
        err,
      );
  ...
  const triggerScan = async (base64Image?: string, mime?: string) => {
    if (!base64Image) {
      setScanError(
        "Please upload an image or start the live camera to capture a nutrition label.",
      );
      return;
    }
    ...
    try {
      ...
      setScanError(
        err.message || "An error occurred while analyzing the label.",
      );
    } finally {
      setIsScanning(false);
    }
  ...
  const handleClearLogs = () => {
    if (
      window.confirm(
        "Are you sure you want to clear your entire nutrition log for today?",
      )
    ) {
      saveDiary([]);
    }
  ...
  const proteinPercent = Math.min(
    Math.round((dailyTotals.protein / userProfile.proteinGoal) * 100),
    100,
  );
  const carbsPercent = Math.min(
    Math.round((dailyTotals.carbs / userProfile.carbsGoal) * 100),
    100,
  );
  const fatPercent = Math.min(
    Math.round((dailyTotals.fat / userProfile.fatGoal) * 100),
    100,
  );
  const sodiumPercent = Math.min(
    Math.round((dailyTotals.sodium / userProfile.sodiumGoal) * 100),
    100,
  );

  // Apply profile presets
  const applyPreset = (
    type: "weight-loss" | "muscle" | "keto" | "balanced",
  ) => {
      ...
      >
        ...
        <div className="bg-white border-b border-[#E5DCC5] px-5 py-3.5 flex justify-between items-center select-none shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-lg font-serif font-extrabold text-[#4A5D4E] tracking-tight leading-none">
                NutriScan
              </h1>
              <span className="text-[10px] text-[#8C8279] font-bold uppercase tracking-[0.08em] leading-none block mt-0.5">
                Android Core
              </span>
            </div>
          </div>
          ...
          >
            ...
            <span className="text-xs font-bold text-[#2D3033]">
              ...
              <span className="text-[10px] font-normal text-[#8C8279]">
                kcal
              </span>
            </span>
          </div>
                  ...
                  {useRealCamera ? (
                    ...
                    <div className="mx-4 mt-4 bg-white rounded-2xl border border-[#E5DCC5] p-4 shadow-sm flex flex-col gap-3 relative overflow-hidden">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-serif font-extrabold text-[#2D3033]">
                            {t.scanNutritionLabel}
                          </h3>
                  ...
                  <div className="mx-4 mt-4 bg-white rounded-2xl border border-[#E5DCC5] shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 flex flex-col gap-3">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          <div className="inline-block bg-[#F8F5F2] border border-[#E5DCC5] px-2 py-1 rounded-lg">
                            <p className="text-[8px] font-bold text-[#8C8279] uppercase leading-none">
                              {t.calorieEstimate}
                            </p>
                            <p className="text-sm font-black font-mono text-[#C97D60] leading-none mt-1">
                              {foodOfTheDay.calories}{" "}
                              <span className="text-[9px] font-normal text-[#2D3033] font-sans">
                                kcal
                              </span>
                            </p>
                          </div>
                      ...
                      <div className="grid grid-cols-4 gap-2 border-t border-b border-[#F8F5F2] py-2.5 text-center">
                        <div>
                          <p className="text-[8px] font-bold text-[#8C8279] uppercase">
                            {t.protein}
                          </p>
                          <p className="text-xs font-bold font-mono text-[#7D8F69] mt-0.5">
                            {foodOfTheDay.protein}g
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-[#8C8279] uppercase">
                          ...
                          <p className="text-xs font-bold font-mono text-[#8C8279] mt-0.5">
                            {foodOfTheDay.carbs}g
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-[#8C8279] uppercase">
                      ...
                      {WORLD_FOODS.map((food) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <div className="text-right">
                              <p className="text-xs font-bold font-mono text-[#C97D60]">
                                {food.calories} kcal
                              </p>
                  ...
                  {scannedResult && !showResultDetail && (
                    <div className="mx-4 mb-4 p-3 bg-white border border-[#E5DCC5] rounded-xl flex items-center justify-between shadow-md">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-xs font-bold text-[#2D3033] line-clamp-1">
                            {scannedResult.productName}
                          </p>
                          <p className="text-[10px] text-[#8C8279]">
                            {scannedResult.calories} kcal per serving
                          </p>
                        </div>
                      </div>
                      <button
                ...
                <div className="grid grid-cols-12 gap-4 items-center mb-4 pb-4 border-b border-[#F8F5F2]">
                  ...
                  <div className="col-span-7 space-y-2.5">
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] font-bold text-[#2D3033] mb-0.5">
                        <span>
                          {t.carbs} ({Math.round(dailyTotals.carbs)}g)
                        </span>
                        <span>{carbsPercent}%</span>
                      </div>
                      <div className="w-full h-1.5 bg-[#F8F5F2] rounded-full overflow-hidden">
                        <div
                          className="h-full bg-[#E5DCC5] rounded-full"
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] font-bold text-[#2D3033] mb-0.5">
                        <span>
                          {t.fat} ({Math.round(dailyTotals.fat)}g)
                        </span>
                ...
                <div className="bg-[#F8F5F2] px-3.5 py-2.5 rounded-xl border border-[#E5DCC5] flex items-center justify-between text-[11px]">
                  <div className="flex items-center gap-1.5 text-[#2D3033] font-bold">
                    ...
                    <span className="font-mono text-xs">
                      {dailyTotals.sodium} mg
                    </span>
                    <span className="text-[#8C8279] font-normal">
                      / {userProfile.sodiumGoal} mg
                    </span>
                  </div>
                  <span
              ...
              <div className="flex-1 bg-white rounded-2xl border border-[#E5DCC5] p-4 flex flex-col shadow-sm">
                ...
                {diaryItems.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-6 text-slate-400">
                    ...
                    <p className="text-sm font-bold text-[#2D3033]">
                      {t.diaryEmpty}
                    </p>
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex gap-1.5 mt-1.5 text-[9px] font-mono font-medium text-slate-600">
                            <span className="bg-white border border-[#E5DCC5]/30 px-1 py-0.2 rounded text-emerald-800">
                              P: {item.proteinTotal}g
                            </span>
                            <span className="bg-white border border-[#E5DCC5]/30 px-1 py-0.2 rounded text-amber-800">
                              C: {item.carbsTotal}g
                            </span>
                            <span className="bg-white border border-[#E5DCC5]/30 px-1 py-0.2 rounded text-rose-800">
                              F: {item.fatTotal}g
                            </span>
                          </div>
                        </div>
                ...
                <div className="space-y-3.5">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        saveProfile({ ...userProfile, name: e.target.value })
                      }
              ...
              <div className="bg-white p-4 rounded-2xl border border-[#E5DCC5] shadow-sm space-y-4">
                ...
                <div>
                  <div className="flex justify-between text-xs font-bold text-[#2D3033] mb-1">
                    ...
                    <span className="font-mono text-[#C97D60]">
                      {userProfile.calorieGoal} kcal
                    </span>
                  </div>
                  <input
                ...
                <div>
                  <div className="flex justify-between text-xs font-bold text-[#2D3033] mb-1">
                    ...
                    <span className="font-mono text-[#8C8279]">
                      {userProfile.carbsGoal} g
                    </span>
                  </div>
                  <input
                    type="range"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      saveProfile({
                        ...userProfile,
                        sodiumGoal: Number(e.target.value),
                      })
                    }
                    className="w-full accent-[#7D8F69]"
                  />
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button
                      key={lang.code}
                      onClick={() =>
                        saveProfile({ ...userProfile, language: lang.code })
                      }
            ...
            <div className="max-h-[85%] bg-white rounded-t-[32px] border-t border-[#E5DCC5] flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6">
                  ...
                  <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-2xl flex items-start gap-2.5 text-xs text-rose-900 leading-relaxed">
                    ...
                    <div>
                      <p className="font-extrabold text-rose-950 uppercase tracking-wider text-[10px] mb-0.5 font-sans">
                        {currentLang === "fa"
                          ? "کلید API یافت نشد (حالت شبیه‌ساز)"
                ...
                <div className="flex justify-between items-start gap-3">
                  <div>
                    ...
                    <p className="text-xs text-[#8C8279] font-medium mt-1">
                      Brand:{" "}
                      <span className="font-bold text-[#2D3033]">
                        {scannedResult.brand || "Unknown"}
                      </span>
                    </p>
                    <p className="text-[11px] text-[#8C8279] italic mt-0.5">
                      Suggested serving size:{" "}
                      {scannedResult.servingSize || "1 container"}
                    </p>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    <div className="inline-flex flex-col items-center justify-center p-2 rounded-2xl border bg-[#F8F5F2] border-[#E5DCC5] min-w-16">
                      <span className="text-[9px] font-extrabold text-[#8C8279] uppercase tracking-wide leading-none mb-1">
                        Health Score
                      </span>
                      ...
                      <span
                        className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full mt-1.5 leading-none ${
                          scannedResult.healthRatingLabel.startsWith("A") ||
                          scannedResult.healthRatingLabel.startsWith("B")
                ...
                <div className="bg-[#F8F5F2] p-4 rounded-2xl border border-[#E5DCC5] flex items-center justify-between gap-4">
                  <div>
                    <span className="text-[10px] font-extrabold text-[#8C8279] uppercase tracking-wider block mb-0.5">
                      Portion Multiplier
                    </span>
                  ...
                  <div className="flex items-center gap-1.5 bg-white border border-[#E5DCC5] rounded-xl px-2.5 py-1.5 shadow-sm shrink-0">
                    <button
                      onClick={() =>
                        setPortionServings(
                          Math.max(0.25, portionServings - 0.25),
                        )
                      }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-[#F8F5F2] p-3.5 rounded-2xl border border-[#E5DCC5] shadow-sm relative overflow-hidden">
                    <div className="absolute right-2 top-2 text-[#C97D60] opacity-20">
                      <Flame className="w-8 h-8" />
                    </div>
                    <p className="text-[9px] font-extrabold text-[#8C8279] uppercase mb-0.5">
                      Total Calories
                    </p>
                    <p className="text-2xl font-black text-[#C97D60] font-mono leading-none">
                      {Math.round(scannedResult.calories * portionServings)}
                      <span className="text-xs font-normal text-[#2D3033] font-sans ml-1">
                        kcal
                      </span>
                    </p>
                  </div>
                  ...
                  <div className="bg-[#F8F5F2] p-3.5 rounded-2xl border border-[#E5DCC5] shadow-sm relative overflow-hidden">
                    <div className="absolute right-2 top-2 text-[#7D8F69] opacity-20">
                  ...
                  <div className="bg-[#F8F5F2] p-3.5 rounded-2xl border border-[#E5DCC5] shadow-sm">
                    ...
                    <p className="text-xl font-extrabold text-[#2D3033] font-mono leading-none">
                      ...
                      <span className="text-xs font-normal text-[#8C8279] font-sans ml-1">
                        g
                      </span>
                    </p>
                    <div className="mt-1.5 flex gap-2 text-[8px] text-[#8C8279]">
                      <span>
                        Sat:{" "}
                        {(
                          (scannedResult.saturatedFat || 0) * portionServings
                        ).toFixed(1)}
                        g
                      </span>
                      <span>
                        Trans:{" "}
                        {(
                          (scannedResult.transFat || 0) * portionServings
                        ).toFixed(1)}
                        g
                      </span>
                    </div>
                  ...
                  <div className="bg-[#F8F5F2] p-3.5 rounded-2xl border border-[#E5DCC5] shadow-sm">
                    ...
                    <p className="text-xl font-extrabold text-[#2D3033] font-mono leading-none">
                      {(
                        scannedResult.totalCarbohydrate * portionServings
                      ).toFixed(1)}
                      <span className="text-xs font-normal text-[#8C8279] font-sans ml-1">
                        g
                      </span>
                    ...
                    <div className="mt-1.5 flex gap-2 text-[8px] text-[#8C8279]">
                      <span>
                        Sgrs:{" "}
                        {(
                          (scannedResult.totalSugars || 0) * portionServings
                        ).toFixed(1)}
                        g
                      </span>
                      <span>
                        Fib:{" "}
                        {(
                          (scannedResult.dietaryFiber || 0) * portionServings
                        ).toFixed(1)}
                        g
                      </span>
                    </div>
                ...
                <div className="p-3.5 bg-white border border-[#E5DCC5] rounded-2xl space-y-2 text-xs">
                  <div className="flex justify-between items-center py-1 border-b border-[#F8F5F2]">
                    <span className="text-[#8C8279] font-medium">
                      Sodium Content
                    </span>
                  ...
                  {scannedResult.addedSugars !== undefined && (
                    <div className="flex justify-between items-center py-1">
                      ...
                      <span className="font-extrabold text-[#C97D60] font-mono">
                        {(
                          (scannedResult.addedSugars || 0) * portionServings
                        ).toFixed(1)}{" "}
                        g
                      </span>
                    </div>
                ...
                {scannedResult.vitamins &&
                  scannedResult.vitamins.length > 0 && (
                    <div className="space-y-2">
                      <span className="text-[10px] font-extrabold text-[#8C8279] uppercase tracking-wider block">
                        {t.vitaminsMicro}
                      </span>
                      <div className="grid grid-cols-2 gap-2">
                        {scannedResult.vitamins.map((vit, idx) => (
                          <div
                            key={idx}
                            className="bg-[#F8F5F2] border border-[#E5DCC5]/40 px-3 py-1.5 rounded-xl flex justify-between items-center text-xs"
                          >
                            <span className="font-bold text-[#4A5D4E]">
                              {vit.name}
                            </span>
                            <div className="text-right">
                              <p className="font-extrabold text-[#2D3033] leading-none text-[11px]">
                                {vit.value}
                              </p>
                              {vit.percentDV !== undefined && (
                                <p className="text-[8px] text-[#8C8279] mt-0.5 font-bold leading-none">
                                  {vit.percentDV}% DV
                                </p>
                              )}
                            </div>
                ...
                <div className="space-y-2">
                  {scannedResult.nutritionalHighlights.length > 0 && (
                    <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        {scannedResult.nutritionalHighlights.map(
                          (high, idx) => (
                            <div
                              key={idx}
                ...
                {scannedResult.ingredientsList &&
                  scannedResult.ingredientsList.length > 0 && (
        ...
        <div className="bg-white border-t border-[#E5DCC5] py-2 px-5 flex justify-around items-center shrink-0 shadow-[0_-2px_10px_rgba(0,0,0,0.02)] z-40 relative select-none">
          ...
          >
            <Camera
              className={`w-5 h-5 ${activeTab === "scan" ? "stroke-[2.5]" : "stroke-[1.5]"}`}
            />
          ...
          >
            <BookOpen
              className={`w-5 h-5 ${activeTab === "diary" ? "stroke-[2.5]" : "stroke-[1.5]"}`}
            />
            <span className="text-[10px] font-extrabold uppercase tracking-wider">
              {t.diary}
            </span>
          ...
          >
            <Sliders
              className={`w-5 h-5 ${activeTab === "profile" ? "stroke-[2.5]" : "stroke-[1.5]"}`}
            />
            <span className="text-[10px] font-extrabold uppercase tracking-wider">
              {t.goals}
            </span>
